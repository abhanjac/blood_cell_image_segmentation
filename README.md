...
The network produces an output image which is also **224 x 224** pixels in size and shows the different **Red Blood Cells (RBC)**, **White Blood Cells (WBC)**, and **Platelets or Thrombocytes** in different colors.
...
So overall there are **9** objects and background, so **10** classes in the dataset.
